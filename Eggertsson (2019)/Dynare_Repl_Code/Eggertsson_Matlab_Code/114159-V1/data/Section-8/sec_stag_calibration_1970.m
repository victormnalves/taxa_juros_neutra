function [ obj_val] = sec_stag_calibration_1970(param_vector,other_params,moments)

% This objective function will be minimized during the calibration of the
% 1970 economy 

% We are minimizing over 3 variables: debt limit, theta (profit share), and
% mu (bequest parameter)

    % First, take the parameters from the optimization
    
    dlim_pct = param_vector(1);
    theta = param_vector(2);
        
    % now, set parameters not being optimized over
    
% Other parameters which are not estimated

    gamma = other_params.gamma;
    deprec = other_params.deprec;
    sigma = other_params.sigma;

 % These were already estimated
 
    delta = other_params.delta;
    ...
    % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        theta_1970 = theta*ones(policy.nt+2,1);
        
        theta_2015 =  5.51*ones(policy.nt+2,1);
    ...
    % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            obj_val = 1000*(.01*(nipa_bank_ss.inc_share(1,1) - moments.ls))^2 + 1000*(nipa_bank_ss.debt_inc(1) - moments.debt_inc)^2 ;

% Secular stagnation model with productivity growth (Japan calibration)
%
...
y_ss = 0.90;                % output gap in secular stagnation
pi_ss = 0.9975^20;          % gross inflation rate in secular stagnation
pi_tar = 1.02^20;           % gross inflation target

mu_ss = 1.007^20;           % targeted post-hysteresis productivity growth rate
kappa = log(mu_ss/mu_0)/log(y_ss/y_fe);

%mu_ss = mu_0*(y_ss/y_fe)^kappa;
...
pi_path = pi_ss*exp(IR(:,1));                   % inflation transition path (JP spreadsheet, column C, Figure 5.xlsx)
prod_path = mu_ss*exp(IR(:,2));                 % productivity transition path (JP spreadsheet, column A, Figure 5.xlsx)
out_path = y_ss*exp(IR(:,3));                   % output transition path (JP spreadsheet, column B, Figure 5.xlsx)

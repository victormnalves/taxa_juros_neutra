...
#
# Stage 1 parameter vector: [a_y,1, a_y,2, b_pi, b_y, g, sigma_y~, sigma_pi, sigma_y*, phi]  
#------------------------------------------------------------------------------#
unpack.parameters.stage1 <- function(parameters, y.data, x.data, xi.00, P.00) {
  ...
  A[5, 1]   <- parameters[9]    # phi
  A[6, 1]   <- -parameters[1]*parameters[9] # -a_y,1*phi
  A[6, 2]   <- -parameters[4]*parameters[9] # -b_y*phi
  A[7, 1]   <- -parameters[2]*parameters[9] # -a_y,2*phi

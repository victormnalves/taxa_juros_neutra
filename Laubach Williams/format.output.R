...
format.output <- function(country.estimation, one.sided.est.country, real.rate.country, start, end, run.se = TRUE) {
    output.country <- data.frame(matrix(NA,dim(one.sided.est.country)[1],23))
    ...
    output.country[,5]   <-  country.estimation$out.stage3$y.data[,1] - (country.estimation$out.stage3$potential.filtered * 100)

    output.country[1,7]    <- "Parameter Point Estimates"
    output.country[2,7:16] <- c("a_y1","a_y2","a_r","b_pi","b_y","sigma_ygap","sigma_pi","sigma_ystar","phi","a_y1 + a_y2")
    output.theta.length    <- length(country.estimation$out.stage3$theta)+6
    output.country[3,7:output.theta.length] <- country.estimation$out.stage3$theta
    output.country[3,16]   <- country.estimation$out.stage3$theta[1] + country.estimation$out.stage3$theta[2]
    ...
    if (run.se) {
        ...
        date.se      <- ti(end, tif='quarterly')        # current date in tis format for comparison

        if (date.se >= start.phi.se) {
          output.country[5,7:15] <- country.estimation$out.stage3$se$t.stats
        } else {
    ...
    if (run.se) {
        output.country[,18]   <- seq(from = (as.Date(ti(shiftQuarter(start,-1),'quarterly'))+1), to = (as.Date(ti(shiftQuarter(end,-1),tif='quarterly'))+1), by = 'quarter')
        output.country[,19:21] <- country.estimation$out.stage3$se$se
    }

    output.country[,23] <- real.rate.country[5:length(real.rate.country)] - country.estimation$out.stage3$rstar.filtered

    output.country[,24] <- one.sided.est.country[,4] # adj. output gap

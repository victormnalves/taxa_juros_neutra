...
#
# Stage 3 parameter vector: [a_y,1, a_y,2, a_r, b_pi, b_y, sigma_y~, sigma_pi, sigma_y*, phi]  
#------------------------------------------------------------------------------#
unpack.parameters.stage3 <- function(parameters, y.data, x.data, lambda.g, lambda.z, xi.00, P.00) {
  ...
  A[1, 7]   <- parameters[9]     # phi
  A[1, 8]   <- -parameters[1]*parameters[9] # -a_y,1*phi
  A[1, 9]   <- -parameters[2]*parameters[9] # -a_y,2*phi
  A[2, 8]   <- -parameters[5]*parameters[9] # -b_y*phi
  A         <- t(A)
